...
var Detector = function () {
  function Detector() {
    classNameCallCheck(this, Detector);
  }

  createclassName(Detector, [{
    key: 'phone',
...
/**
 * Adds multiple classNamees on node
 * @param {DOMNode} node
 * @param {array}  classNamees
 */
var addclassNamees = function addclassNamees(node, classNamees) {
  return classNamees && classNamees.forEach(function (className) {
    return node.classNameList.add(className);
  });
};

/**
 * Removes multiple classNamees from node
 * @param {DOMNode} node
 * @param {array}  classNamees
 */
var removeclassNamees = function removeclassNamees(node, classNamees) {
  return classNamees && classNamees.forEach(function (className) {
    return node.classNameList.remove(className);
  });
};
...
/**
 * Set or remove aos-animate className
 * @param {node} el         element
 * @param {int}  top        scrolled distance
 */
var applyclassNamees = function applyclassNamees(el, top) {
  ...
  var hide = function hide() {
    ...
    removeclassNamees(node, options.animatedclassNames);
  ...
  var show = function show() {
    ...
    addclassNamees(node, options.animatedclassNames);
...
/**
 * Scroll logic - add or remove 'aos-animate' className on scroll
 *
...
var handleScroll = function handleScroll($elements) {
  return $elements.forEach(function (el, i) {
    return applyclassNamees(el, window.pageYOffset);
  });
};
...
var prepare = function prepare($elements, options) {
  $elements.forEach(function (el, i) {
    ...
    if (options.initclassName) {
      el.node.classNameList.add(options.initclassName);
    }
...
var disable = function disable() {
  $aosElements.forEach(function (el, i) {
    ...
    if (options.initclassName) {
      el.node.classNameList.remove(options.initclassName);
    }

    if (options.animatedclassName) {
      el.node.classNameList.remove(options.animatedclassName);
    }
  });
};
